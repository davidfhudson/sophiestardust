...
async function submitAttempt(entry) {
  try {
    ...
    if (!res.ok) return { ok: false, firstAttempt: false };
    return await res.json();
  } catch { return { ok: false, firstAttempt: false }; }
}

// Returns true if this player has already submitted a score for this quiz.
// Checks against a pre-fetched attempts array when available (avoids a second round-trip).
function hasPlayedQuiz(name, quizId, attempts) {
